function t_sldsInit(rec, sliderOptions) {
    ...
    if (sliderRec) {
        var sliderWrapper = sliderRec.querySelector(".t-slds__items-wrapper"), isInit;
        if (sliderWrapper) if (!sliderWrapper.getAttribute("data-slider-initialized")) {
            ...
            if (sliderItems) {
                sliderOptions && sliderOptions.randomSort && (sliderItems = t_slds_randomSortElements(sliderItems));
                var totalSlides = sliderItems.length, firstSlide = sliderItems[0],
                    lastSlide = sliderItems[sliderItems.length - 1], windowWidth = window.innerWidth,
                    ...
                    sliderTransition = parseFloat(sliderWrapper.getAttribute("data-slider-transition")), stopSlider;
                if (Array.prototype.forEach.call(sliderItems, (function (item, index) {
                    ...
                    arrowRight.length && Array.prototype.forEach.call(arrowRight, (function (arrow) {
                    ...
                    sldsBlock.length && Array.prototype.forEach.call(sldsBlock, (function (slide) {
                    ...
                    }))), window.t_slds__isiOS && ("complete" === document.readyState ? t_slds__hideMobileSlides(sliderRec.querySelector(".t-slds__item_active")) : window.addEventListener("load", (function () {
...
function t_slds__removeAutoplayByVideo(e) {
    ...
    if (videoSlide) {
        var sliderWrapper = videoSlide.closest(".t-slds__items-wrapper"), galleryIntervalIdAttr;
...
function t_slds__initFeedsSlider(rec, sliderRec) {
    ...
    if (sliderWrapper) {
        var sliderItems = sliderRec.querySelectorAll(".t-slds__item:not(.t-slds__item_dummy)"),
            ...
            firstSlide = sliderItems[0], lastSlide = sliderItems[sliderItems.length - 1],
            sliderWithCycle = sliderWrapper.getAttribute("data-slider-with-cycle");
...
function t_slds__createDummies(firstSlide, lastSlide, sliderWrapper) {
    var sliderItems, totalSlides = sliderWrapper.querySelectorAll(".t-slds__item:not(.t-slds__item_dummy)").length,
        itemsInRow = parseInt(sliderWrapper.getAttribute("data-slider-items-in-row"), 10) || 0,
        sliderWithCycle = sliderWrapper.getAttribute("data-slider-with-cycle"),
        isFeedsSlider = sliderWrapper.classList.contains("js-feed-container"), restoreAttrClassFirst = !1,
    ...
    if (lastSlide && 0 == sliderWrapper.querySelectorAll('.t-slds__item[data-slide-index="0"]').length) {
        ...
        fieldItem.length && Array.prototype.forEach.call(fieldItem, (function (item) {
...
function t_slds_updateFeedsSliderOnResize(rec, sliderRec, sliderOptions) {
    var sliderWrapper = sliderRec.querySelector(".t-slds__items-wrapper"), isFeedsSlider;
...
function t_slds_setItemsInRow(rec, defaultCountItemsInRow) {
    var sliderRec = "object" == typeof rec ? rec[0] : document.querySelector("#rec" + rec);
    if (sliderRec) {
        var sliderWrapper = sliderRec.querySelector(".t-slds__items-wrapper"), itemsInRow, updatedItemsInRow;
    ...
    if (sliderRec) {
        ...
        if (sliderWrapper) {
            ...
            if ("true" !== sliderWrapper.getAttribute("data-slider-stop")) {
                ...
                arrowWrapper.length && Array.prototype.forEach.call(arrowWrapper, (function (wrapper) {
                    wrapper.addEventListener("click", (function () {
                        var currentTranslate = t_slds_getCurrentTranslate(sliderRec),
                            ...
                            isCycled = sliderWrapper.getAttribute("data-slider-with-cycle"), cycle = "", direction;
                ...
                bullets.length && Array.prototype.forEach.call(bullets, (function (bullet) {
                    bullet.addEventListener("click", (function () {
                        var newPosition = parseInt(bullet.getAttribute("data-slide-bullet-for"), 10), currentPosition;
...
function t_slideMoveWithoutAnimation(rec, withoutNewInterval, sliderOptions) {
    ...
    if (sliderRec) {
        var sliderWrapper = sliderRec.querySelector(".t-slds__items-wrapper");
        if (sliderWrapper) {
            var position = parseFloat(sliderWrapper.getAttribute("data-slider-pos")), itemsInRow,
                sliderWidth = (sliderWrapper.getAttribute("data-slider-items-in-row") || 0) > 0 ? sliderRec.querySelector(".t-slds__container .t-slds__item").offsetWidth : sliderRec.querySelector(".t-slds__container").offsetWidth,
                totalSlides = parseFloat(sliderWrapper.getAttribute("data-slider-totalslides")),
            ...
            if (position > totalSlides + 1 && (position = 1), sliderNotAnimated) {
                ...
                if (activeSlide && 0 === sliderRec.querySelectorAll(".t-slds__item_dummy").length) {
                    ...
                    if (dummy.classList.add("t-slds__item_dummy"), dummy.style.position = "absolute", dummy.style.left = sliderWidth * position + "px", sliderWrapper.appendChild(dummy), sliderRec.classList.contains("js-product")) catalog = sliderRec; else var catalog = sliderRec.querySelector(".t-store") || sliderRec.querySelector(".js-product");
    ...
    if (sliderRec) {
        ...
        if (sliderWrapper) {
            var sliderItems = sliderRec.querySelectorAll(".t-slds__item:not(.t-slds__item_dummy)"),
                ...
                arrowWrapperLeft = sliderRec.querySelector(".t-slds__arrow_wrapper-left"), ariaLiveAttr;
            ...
            var isiOSChrome = !!navigator.userAgent.match("CriOS"), stopSlider;
            if (sliderTransition || 0 === sliderTransition || (sliderTransition = 300), position > totalSlides + 1 && (position = 1, sliderWrapper.setAttribute("data-slider-pos", 1)), "true" !== sliderWrapper.getAttribute("data-slider-stop")) {
                if (arrowWrapperRight) if (isFeedsSlider) {
...
function t_slds_ActiveSlide(rec, position, totalSlides) {
    ...
    if (sliderRec) {
        var sliderWrapper = sliderRec.querySelector(".t-slds__items-wrapper"),
            ...
            iframes = sliderRec.querySelectorAll("iframe"), videos = sliderRec.querySelectorAll("video");
        ...
        })), 0 == position && !1 === sliderNotAnimated) sliderRec.querySelector('.t-slds__item[data-slide-index="' + totalSlides + '"]').classList.add("t-slds__item_active"), sliderRec.setAttribute("aria-hidden", "false"); else if (0 == position && !0 === sliderNotAnimated) slideActive = sliderRec.querySelector('.t-slds__item[data-slide-index="' + totalSlides + '"]'); else if (position == totalSlides + 1 && !1 === sliderNotAnimated) {
    ...
    if (sliderRec) {
        ...
        if (sliderContainer) {
            var containerPaddingLeft = parseInt(getComputedStyle(sliderContainer).paddingLeft) || 0,
                ...
                sliderWrapper = sliderRec.querySelector(".t-slds__items-wrapper"), stopSlider;
            if ("true" != sliderWrapper.getAttribute("data-slider-stop")) {
                var itemsInRow = 0;
                sliderWrapper && (itemsInRow = sliderWrapper.getAttribute("data-slider-items-in-row"), sliderWrapper.style.width = sliderContainerWidth * totalSlides + "px"), window.innerWidth <= 640 ? itemsInRow = 1 : window.innerWidth <= 960 && itemsInRow > 1 && (itemsInRow = 2);
...
function t_slds_SliderArrowsHeight(rec) {
    ...
    if (sliderRec) {
        ...
        if (activeItem) {
            var offsetForVideoBtns = 0, isVideo;
        ...
        height && arrowWrappers.length && Array.prototype.forEach.call(arrowWrappers, (function (arrowWrapper) {
...
function t_slds_initAutoPlay(rec, position, totalSlides, sliderOptions) {
    var isZeroBlock = "object" == typeof rec, sliderRec = isZeroBlock ? rec[0] : document.querySelector("#rec" + rec);
    if (sliderRec) {
        var sliderContainer = sliderRec.querySelector(".t-slds"),
            sliderWrapper = sliderRec.querySelector(".t-slds__items-wrapper");
        if (sliderWrapper) {
            var sliderTimeOut = parseFloat(sliderWrapper.getAttribute("data-slider-timeout")), cycle = "",
                galleryIntervalIdAttr = sliderWrapper.getAttribute("data-slider-interval-id"), stopSlider;
            if (galleryIntervalIdAttr && clearInterval(galleryIntervalIdAttr), "true" !== sliderWrapper.getAttribute("data-slider-stop")) {
                var isMobile;
                ...
                window.addEventListener("resize", t_throttle((function () {
                    ...
                    if (document[hidden]) sliderWrapper.setAttribute("data-slider-stopped", "yes"); else {
                        var display = getComputedStyle(sliderRec).display, viewportTop = window.pageYOffset,
...
function t_slds_initSliderSwipe(rec, totalSlides, windowWidth, sliderOptions) {
    ...
    if (sliderRec) {
        ...
        if (sliderMain) {
            var sliderWrapper = sliderRec.querySelector(".t-slds__items-wrapper");
            if (sliderWrapper) {
                var timeout, isScrolling = !1, isSwiping = !1, stopSlider, isSwiperInit;
                if ("true" !== sliderWrapper.getAttribute("data-slider-stop")) if ("true" !== sliderWrapper.getAttribute("data-swiper-initialized")) {
                    ...
                    }, stopAttribute = "ontouchend" in document ? "data-slider-touch" : "data-slider-stopped", observer;
                    ...
                    })).observe(sliderRec); else stopSliderIfNeed(stopAttribute), window.addEventListener("scroll", (function () {
                    ...
                    if (1 != totalSlides) {
                        ...
                        window.removeEventListener("touchmove", scrollPrevent, {passive: !1}), window.addEventListener("touchmove", scrollPrevent, {passive: !1}), hammer.on("pan", (function (event) {
                            ...
                            var sliderWrapper = sliderRec.querySelector(".t-slds__items-wrapper"),
                                ...
                                position = parseFloat(sliderWrapper.getAttribute("data-slider-pos")),
                                totalSlides = parseFloat(sliderWrapper.getAttribute("data-slider-totalslides")),
                                cycle = "", distance = event.deltaX,
                                percentage = 100 / totalSlides * event.deltaX / window.innerWidth, sensitivity = 30,
                                stopSlider;
                            if ("true" !== sliderWrapper.getAttribute("data-slider-stop") && (sliderWrapper.setAttribute("data-slider-touch", "yes"), t_slds_scrollImages(rec, sliderWidth * position - distance), event.isFinal)) {
                                if (event.velocityX > .4) "false" == sliderWrapper.getAttribute("data-slider-with-cycle") && 1 == position ? position = 1 : position--, sliderWrapper.setAttribute("data-slider-pos", position), 0 == position && (cycle = "yes"), sliderWrapper.setAttribute("data-slider-cycle", cycle), t_slideMove(rec, !1, sliderOptions); else if (event.velocityX < -.4) {
                                    if ("false" == sliderWrapper.getAttribute("data-slider-with-cycle") && (position == totalSlides || withSingleMove && position == totalSlides - itemsInRow + 1)) position = withSingleMove ? totalSlides - itemsInRow : totalSlides; else {
                                        var sliderItems = sliderRec.querySelectorAll(".t-slds__item:not(.t-slds__item_dummy)"),
...
function t_slds_changeImageUrl(rec) {
    var allRec = document.getElementById("allrecords"),
        ...
        sliderRec = "object" == typeof rec ? rec[0] : document.querySelector("#rec" + rec), imgs;
...
function t_slds_fadeIn(el, duration, complete) {
    if (("1" !== getComputedStyle(el).opacity && "" !== getComputedStyle(el).opacity || "none" === getComputedStyle(el).display) && !el.getAttribute("data-fadein-timeout")) {
        var opacity = 0, speed = (duration = parseInt(duration)) > 0 ? duration / 10 : 40;
...
function t_slds_randomSortElements(sliderItems) {
    var itemsArray = Array.prototype.slice.call(sliderItems);
    if (itemsArray.length) {
        itemsArray.sort((function () {
            return .5 - Math.random()
        }));
        var parentContainer = sliderItems[0].parentNode;
        return itemsArray.forEach((function (item) {
