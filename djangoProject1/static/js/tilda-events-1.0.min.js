window.Tilda = window.Tilda || {}, function () {
    function t_events__initEvent() {
        ...
        function linkClickCreateEvent(e) {
            ...
            if (e.target && targetEl) {
                var virtPage = targetEl.getAttribute("data-tilda-event-name"), virtTitle = targetEl.textContent,
                    url = targetEl.getAttribute("href") || "", target = targetEl.getAttribute("target");
                if (!virtPage) {
    ...
    Tilda.sendEcommerceEvent = function (type, productsArr) {
        ...
        for (var virtPage, virtTitle = "", virtPrice = 0, virtProducts = [], variant_str, productObj, recid = "", uid = "", lid = "", i = 0; i < productsArr.length; i++) {
            ...
            var virtProduct = {name: productObj.name, price: productObj.price, variant: variant_str, quantity: 1};
        ...
        var noVkSendEvent = !(!allRecVkSendEvent || "nosend" !== allRecVkSendEvent), currencyCode = "",
            curBlock = document.querySelector(".t706");
        if (currencyCode = allRec.getAttribute("data-tilda-currency") ? allRec.getAttribute("data-tilda-currency") : curBlock && curBlock.getAttribute("data-project-currency-code") ? curBlock.getAttribute("data-project-currency-code") : "RUB", referer || (referer = window.location.href), (price = price ? parseFloat(price) : 0) > 0) if (window.dataLayer || (window.dataLayer = []), -1 !== virtPage.indexOf("/tilda/") && -1 !== virtPage.indexOf("/payment/") && window.tildaForm && window.tildaForm.orderIdForStat > "") referer = {
            ecommerce: {
                purchase: {
                    actionField: {
        ...
        try {
            var isOldCounter;
            if (window.gtagTrackerID && "gtag" === window.mainTracker) if (isVirtPage) if (referer && referer.event) if ("purchase" === referer.event) {
                for (var products = referer.ecommerce.purchase.products, delivery = 0, i = 0; i < products.length; i++) if ("delivery" === products[i].id) {
                    delivery = products[i];
                    break
                }
                gtag("event", "purchase", {
            ...
            }); else gtag("event", virtPage, {event_category: "tilda", event_label: virtTitle, value: price})
        } catch (e) {
        }
        if (window.ga && "tilda" !== window.mainTracker && "gtag" !== window.mainTracker) if (isVirtPage) if (referer && referer.event) {
            try {
        ...
        } else ga("send", {hitType: "pageview", page: virtPage, title: virtTitle}); else ga("send", {
            hitType: "event",
            eventCategory: "tilda",
            eventAction: virtPage,
        ...
        if (window.mainMetrikaId && window.mainMetrikaId > 0 && "function" == typeof ym) if (isVirtPage) {
            var mOpts = {title: virtTitle};
            price > 0 ? (mOpts.params = {order_price: price}, currencyCode && (mOpts.params.currency = currencyCode), ym(window.mainMetrikaId, "hit", virtPage, mOpts)) : ym(window.mainMetrikaId, "hit", virtPage, mOpts)
        } else price > 0 ? (mOpts = {order_price: price}, currencyCode && (mOpts.currency = currencyCode), ym(window.mainMetrikaId, "reachGoal", virtPage, mOpts)) : ym(window.mainMetrikaId, "reachGoal", virtPage);
        if (window.mainMetrika > "" && window[window.mainMetrika] && (isVirtPage ? price > 0 ? window[window.mainMetrika].hit(virtPage, {
            title: virtTitle,
            order_price: price,
            params: referer
        }) : window[window.mainMetrika].hit(virtPage, {title: virtTitle}) : price > 0 ? window[window.mainMetrika].reachGoal(virtPage, {
            title: virtTitle,
            params: referer,
            order_price: price
            ...
            if (isVirtPage) if (-1 === virtPage.indexOf("tilda/") || -1 === virtPage.indexOf("/payment/") && -1 === virtPage.indexOf("/submitted/")) if (referer && referer.event && price > 0) if ("addToCart" === referer.event && referer.ecommerce.add) {
                ...
                content_name: virtTitle,
                content_category: virtPage
            }); else window.fbq("track", virtPage, {content_name: virtTitle, value: price})
        } catch (e) {
        }
        if (void 0 !== window.VK && void 0 !== window.VK.Retargeting && !1 === noVkSendEvent) try {
            if (isVirtPage) {
                var allRec,
                    vkPriceListId = (allRec = document.getElementById("allrecords")) ? allRec.getAttribute("data-vk-price-list-id") : null,
                    priceListID = vkPriceListId && parseInt(vkPriceListId) || 0, eventName = "", eventParams = !1, tmp,
                    tmp, tmp, tmp, tmp;
                if (referer && referer.event) if (eventParams = {
                    products: [],
                    currency_code: "",
                    total_price: 0
                }, "purchase" === referer.event && referer.ecommerce.purchase) if (price > 0 && priceListID > 0) {
                    var product;
                ...
                } else eventName = "t-purchase"; else if ("addToCart" === referer.event && referer.ecommerce.add) if (price > 0 && priceListID > 0) {
                    var product;
                    eventParams.currency_code = currencyCode;
                ...
                } else eventName = "t-add-to-cart", referer.ecommerce.add[0] && referer.ecommerce.add[0].uid && (eventName += "-" + referer.ecommerce.add[0].uid); else if ("viewProduct" === referer.event && referer.ecommerce.detail) if (price > 0 && priceListID > 0) {
                    var product;
                    eventParams.currency_code = currencyCode;
                ...
                } else eventName = "t-view-product", referer.ecommerce.detail[0] && referer.ecommerce.detail[0].uid && (eventName += "-" + referer.ecommerce.detail[0].uid); else if ("removeFromCart" === referer.event && referer.ecommerce.remmove) if (price > 0 && priceListID > 0) {
                    var product;
                    eventParams.currency_code = currencyCode;
        ...
        "function" == typeof window.tildastat && (isVirtPage ? (virtPage.indexOf("payment") > 0 && virtPage.indexOf("tilda/form") > -1 && (virtPage = virtPage.replace("tilda/form", "tilda/rec")), window.tildastat("pageview", {page: virtPage})) : window.tildastat("pageview", {page: "/tilda/event/" + virtPage}))
    }, Tilda.saveUTM = function () {
        try {
            var tildaPageURL = window.location.href, tildaPageQuery = "", tildaPageUTM = "";
